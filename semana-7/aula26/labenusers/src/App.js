...
import {
  baseUrl,
  axiosConfig
} from './assets/components/axiosConstants'
...
export default class App extends React.Component {
  ...
  detailUser = async (user) => {
    try {
      const response = await axios.get(`${ baseUrl }/${ user.id }`, axiosConfig)
      this.setState({
  ...
  editUser = async (user) => {
    ...
    try {
      await axios.put(`${ baseUrl }/${user.id}`, body, axiosConfig)
